...
static void NormalizeBlend(CFloatImage& acc, CByteImage& img)
{
    ...
    for (y = 0; y < sh.height; y++)
    {
        ...
        {
            double weight = acc.Pixel(x,y,3);
            if (weight > 0){
                //divide by weights
                img.Pixel(x,y,0)=acc.Pixel(x,y,0)/weight;
                img.Pixel(x,y,1)=acc.Pixel(x,y,1)/weight;
                img.Pixel(x,y,2)=acc.Pixel(x,y,2)/weight;
                //set alpha to opaque
                img.Pixel(x,y,3)=255;
...
CByteImage BlendImages(CImagePositionV& ipv, float blendWidth)
{
    ...
    A[0][1] = 0;
    A[0][2] = width/2;
    A[1][0] = (y_init-y_final)/(x_init-x_final);
    A[1][1] = 1;
    A[1][2] = 0;
    A[2][0] = 0;
    A[2][1] = 0;
    ...
    //A = CTransform3x3::Translation(0.5*width,y_final-y_init);
    //A[1][0] = (M_start[1][2]-M_end[1][2])/(M_start[0][2]-M_end[0][2]);
    //A = CTransform3x3::Translation(0.5*width,pos_end[1] - min_y);
    //A[1][0] = (y_init-y_final)/(x_init-x_final);
	// *** END TODO #2 ***
